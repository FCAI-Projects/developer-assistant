...
interface SetDeadlineModelProps {
  deadline: string;
  handleUpdateTask: (field: string, value: string) => void;
}

export const SetDeadlineModel: React.FC<SetDeadlineModelProps> = ({ handleUpdateTask, deadline }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, formikApi) => {
      try {
        handleUpdateTask("deadline", values.date);
        toggleModal();
      } catch (error) {
        console.log(error);
      }
